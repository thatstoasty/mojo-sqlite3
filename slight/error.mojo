...
fn error_msg(db: UnsafePointer[sqlite3_connection], code: SQLite3Result) -> Optional[StringSlice[ImmutableAnyOrigin]]:
    ...
    if not db or get_sqlite3_handle()[].errcode(db) != code:
        if err_str := get_sqlite3_handle()[].errstr(code.value):
            return StringSlice(unsafe_from_utf8_ptr=err_str)
        return None

    return StringSlice(unsafe_from_utf8_ptr=get_sqlite3_handle()[].errmsg(db))


fn raise_if_error(db: UnsafePointer[sqlite3_connection], code: SQLite3Result) raises:
...
fn decode_error(db: UnsafePointer[sqlite3_connection], code: SQLite3Result) -> Error:
